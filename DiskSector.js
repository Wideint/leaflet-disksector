 ...
 * @inherits Circle
 *
 * Original code by Jan Pieter Waagmeester <jieter@jieter.nl>.
 * A class for drawing disk sectors overlays on a map. Extends `Circle`.
 */
...
export var DiskSector = L.DiskSector = L.Circle.extend({
    options: {
        startAngle: 0,
        stopAngle: 359.9999
    },
...
L.SVG.include({
    _updateDiskSector: function (layer) {
        // If we want a circle, we use the original function
        //if (!(layer instanceof L.Sector) || !layer.isSector()) {
        //    return _updateCircle.call(this, layer);
        //}
        if (layer._empty()) {
